...
import { whatsAppSchema } from "../(server)/utils/whatsAppSchema";
import getWhatsAppEdit from "@/app/whatsapp/(server)/actions";
...
export default function WhatsApp() {
    ...
    const mutation = useMutation({
        mutationFn: (data: formField) => {
            ...
            return getWhatsAppEdit(modifiedData)
        },
        onSuccess: (data) => {
            const imageUrlProfile = `data:image/png;base64,${data.screenshotBufferProfile}`;
            const imageUrlSettings = `data:image/png;base64,${data.screenshotBufferSettings}`;
            setImgUrls([imageUrlProfile, imageUrlSettings]);
    ...
    return (
        <Card className="flex flex-col gap-4 overflow-hidden rounded-none p-4 select-none max-w-md mx-auto">
            ...
            <form className="flex flex-col gap-1" onSubmit={handleSubmit(handleFormSubmit)}>
                ...
                />
                {formState.errors.whatsAppName && <span className="text-red-500 text-sm">{formState.errors.whatsAppName?.message}</span>}
                <Input
                    type="text"
                ...
                />
                {formState.errors.whatsAppNumber && <span className="text-red-500 text-sm">{formState.errors.whatsAppNumber?.message}</span>}
                <Input
                    type="text"
                    placeholder="WhatsApp About"
                    {...register("whatsAppAbout")}
                />
                {formState.errors.whatsAppAbout && <span className="text-red-500 text-sm">{formState.errors.whatsAppAbout?.message}</span>}
                <div className="grid grid-cols-[2fr_1fr] gap-2">
                    <Input
                        type="file"
                        accept="image/*"
                        onChange={(e) => setValue("whatsAppProfilePic", e.target.files?.[0])}
                    />
                    <Input
                        type="time"
                        {...register("time")}
                ...
                {formState.errors.whatsAppProfilePic && <span className="text-red-500 text-sm">{formState.errors.whatsAppProfilePic?.message}</span>}
