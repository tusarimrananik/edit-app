...
import { facebookCookies } from '../facebook/(server)/utils/facebookCookie';
import { scrapeFacebook } from '../facebook/(server)/utils/scrapeFB';
import setDataTakeSS from '../facebook/(server)/utils/setDataTakeSS';
import { facebookLinkSchema } from '../facebook/(server)/utils/facebookLinkSchema';
import { auth } from "@/auth";
import prisma from '@/lib/prisma';
import { ServerStatus } from '@/app/utils/ServerStatus';
import { getActiveBrowser } from "@/app/utils/getAndSetBrowsers"

export default async function getFacebookInfo(data: { facebookLink: string, time: string }) {
    ...
    try {
        ...
        ServerStatus.setBusy(false);
        return { scrapedData, userId: session.user.id };
    } catch (error) {
        ServerStatus.setBusy(false);
        const errorMessage =
